...
namespace Rock.Rest.v2
{
    ...
    {
        #region Small Group Dropdown

        /// <summary>
        /// Returns a list of groups (Guid, Name, IsActive), optionally including inactive items.
        /// </summary>
        /// <param name="includeInactive">If true, returns all groups; if false, only active groups.</param>
        [HttpGet]
        [Route("GetGroupDropdownOptions")]
        [Authenticate]
        [ProducesResponseType(HttpStatusCode.OK, Type = typeof(List<GroupDropdownOption>))]
        public IActionResult GetGroupDropdownOptions(bool includeInactive = false)
        {
            using (var rockContext = new RockContext())
            {
                var groupService = new GroupService(rockContext);
                var groupsQuery = groupService.Queryable();
                ...
                var result = groupsQuery
                    .Select(g => new GroupDropdownOption
                    {
                        Guid = g.Guid,
                        Name = g.Name,
                        IsActive = g.IsActive
...
");
    ...
    #region GetGroupDropDownOption
    /// <summary>
    /// Simple DTO for returning group Guid/Name/IsActive to the front end.
    /// </summary>
    public class GroupDropdownOption
    {
